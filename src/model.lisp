...
(defun create-user-table ()
  "Create user talble if it doesn't exit yes."
  (with-connection (db)
    (execute
...
(defun add-page (title author content)
  "Add page to dtabase. Mark it as \"latest: true\", mark previous \"latest: false\"."
  (with-transaction (db)
    (execute
     (update :page
       (set= :latest "false")
...
(defun get-latest-page (title)
  "Get latest version of a page by TITLE."
  (with-connection (db)
    (retrieve-one
     (select :*
       (from :page)
...
(defun get-latest-pages-by-user (username)
  "Get the latest version of all pages by USERNAME."
  (with-connection (db)
    (retrieve-all
...
(defun get-sorted-pages (title)
  "Get all version of a page by TITLE, sorted by it's timestamp in descending order, newest first."
  (with-connection (db)
    (retrieve-all
     (select :*
       (from :page)
...
(defun create-tables ()
  "Create all tables, if they don't exit already."
  (create-user-table)
  (create-page-table))